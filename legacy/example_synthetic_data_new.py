...
MSE_const = np.zeros((105, 3))
MSE_unconst = np.zeros((105, 3))
MSE_combine = np.zeros((105, 3))
...
for t in range(0, 3):
    ...
    for j in range(0, 105, 1):
        ...
        for i in range(0, N):
            ...
            # Combine
            a = 1/(1+np.exp(-K*(maha_distance-this_T)))
            b = 1 - a
            y_predict_combine[i] = a*y_predict_const[i] + b*y_predict_unconst[i]
            
        # MSE
        MSE_const[j, t] = mean_squared_error(y_true, y_predict_const) 
        MSE_unconst[j, t] = mean_squared_error(y_true, y_predict_unconst) 
        MSE_combine[j, t] = mean_squared_error(y_true, y_predict_combine) 

# =============================================================================
# Visualization of results    
# =============================================================================
plt.figure(figsize=(5, 3.5))
plt.plot(np.arange(-10, 11, .2), MSE_combine[:, 0].reshape(105), 'g:', label=r"$d_0 = 0.1$") 
plt.plot(np.arange(-10, 11, .2), MSE_combine[:, 1].reshape(105), 'g-', label=r"$d_0 = 0.5$") 
plt.plot(np.arange(-10, 11, .2), MSE_combine[:, 2].reshape(105), 'g--', label=r"$d_0 = 1$") 
plt.xlim((-10, 10))
plt.ylim((0, 10))
plt.legend(loc=2)   
plt.xlabel("$z$")
