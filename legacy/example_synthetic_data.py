...
#plt.figure(figsize=(5, 5))
plt.plot(z1, np.log(rz.pdf(z1)), 'k-', lw=2, label='Normal')
#plt.plot(z_x.reshape(100), rz_x.pdf(z_x).reshape(100), 'k--', lw=2, label='$p(z_* | x_* )$')
plt.plot(z, log_p_z,  'k--', label='Fat-tailed')
plt.xlim((-10, 10))
plt.ylim((-10, 10))
...
plt.plot(np.arange(-10, 11, .2), MSE_combine.reshape(105), 'k-', label=r"$a\bm{w}_{MMSE} + (1-a)\bm{w}_{C}$") 
plt.plot(np.arange(-10, 11, .2), MSE_unconst.reshape(105), 'k--', label=r"$\bm{w}_{MMSE}$") 
plt.plot(np.arange(-10, 11, .2), MSE_const.reshape(105), 'k:', label=r"$\bm{w}_{C}$")
plt.xlim((-10, 10))
plt.ylim((0, 10))
plt.legend(loc=2)   
plt.xlabel("$z_*$")
...
#plt.savefig("d_1_K_100.pdf", bbox_inches='tight')
