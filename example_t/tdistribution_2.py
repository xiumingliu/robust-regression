...
#            x = np.random.multivariate_normal(mu_x_z.reshape(d), Cov_x_z)
            ...
            y_true[i] = np.dot(np.array([.5, .5, .5, .5]), x[0, :]) + 0.5*z
            
            # Robust 
            y_predict_const[i] = np.dot(np.transpose(w_c), x[0, :].reshape(d,1))
            
            # LMMSE
            y_predict_unconst[i] = np.dot(np.transpose(w_opt), x[0, :].reshape(d,1))
            
            z_predict[i] = np.dot(np.transpose(w_z), x[0, :].reshape(d,1))
...
z = np.linspace(rv.ppf(0.0001), rv.ppf(0.9999), 1000)
p_z = rv.pdf(z) 
