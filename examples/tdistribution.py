...
#plt.plot(np.arange(-10, 11, .2), MSE_combine_median.reshape(105), 'g-', label=r"$\widehat{\Pr}\ (z \in \mathcal{Z}_{\alpha}\ |\ \bm{x})\bm{w}_{c} + $"+"\n"+ r"$\widehat{\Pr}\ (z \notin \mathcal{Z}_{\alpha}\ |\ \bm{x})\bm{w}_{o}$")
plt.plot(np.arange(-10, 11, .2), MSE_combine_median.reshape(105), 'g-', label=r"$\widehat{\bm{w}}(\bm{x})$")

plt.fill_between(np.arange(-10, 11, .2), MSE_unconst_25, MSE_unconst_75, alpha = 0.5, color ='r')
plt.plot(np.arange(-10, 11, .2), MSE_unconst_median.reshape(105), 'r--', label=r"$\widehat{\bm{w}}_{o}$")

plt.fill_between(np.arange(-10, 11, .2), MSE_const_25, MSE_const_75, alpha = 0.5, color ='b')
plt.plot(np.arange(-10, 11, .2), MSE_const_median.reshape(105), 'b:', label=r"$\widehat{\bm{w}}_{c}$")
