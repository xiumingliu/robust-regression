...
for iter in range(iter_max):
    ...
    for n in range(N):
        ...
        xyz[:, n] = np.random.multivariate_normal(mu[:, k].reshape(q+d+1), Sigma)
    
#    XZ = xyz[0:d+q, :]
    X = xyz[0:d, :].reshape(d, N)
    Z = xyz[d+1:d+1+q, :].reshape(q, N)
    Y = xyz[d:d+1, :].reshape(1, N)
...
#                        [.25, .25, .5, 1]])
    ...
    for j in range(0, 105, 1):
        ...
        pdf_z = np.zeros(Cat)
        for k in range(Cat):
            pdf_z[k] = norm.pdf(z, mu[d+1:d+1+q, k], 1)*pi[k]
        pdf_z = np.sum(pdf_z)    
        
        
        for k in range(Cat):
            this_mu_new, Cov_xy_z = conditional_mean(z, mu[d+1, k], mu[0:d+1, k].reshape((d+1, 1)), Sigma)
            mu_xy_z[:, k] = this_mu_new.reshape((d+1))
            pi_xy_z[k] = norm.pdf(z, mu[d+1:d+1+q, k], 1)*pi[k]/pdf_z
...
#            maha_distance = distance.mahalanobis(z_predict[i], 0, 1)
            ...
            b = 1 - a
            y_predict_combine[i] = b*y_predict_const[i] + a*y_predict_unconst[i]
